...
const DashboardLayout = () => {
  ...
  const fetchFavorites = async () => {
    ...
    try {
      const response = await axios.get(`https://localhost:7231/api/FileUploadAPI/${user.id}/likes`);
  ...
  const handleFavorite = async (e, mockupId) => {
    ...
    try {
      ...
      await axios.post(`https://localhost:7231/api/FileUploadAPI/${user.id}/like/${mockupId}`, true, {
        headers: {
