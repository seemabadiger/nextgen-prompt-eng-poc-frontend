...
import { BsCloudUpload, BsLink45Deg, BsFillTrash3Fill } from "react-icons/bs";
...
const UploadMockupModal = ({
  ...
  const validationSchema = Yup.object().shape({
    // mockuptype: Yup.string().required("Required"),
  ...
  const handleTagChange = (index, tag, values, type, setFieldValue) => {
    const mockups = values[type];
    mockups[index].tags = mockups[index].tags.includes(tag)
      ? mockups[index].filter((t) => t !== tag)
      : [...mockups[index].tags, tag];
  ...
  const handleTagDataChange = (tag, values, type, setFieldValue) => {
    const mockups = values[type];
    const tags = mockups.includes(tag)
      ? mockups.filter((t) => t !== tag)
      : [...mockups, tag];
  ...
  const uploadProcessData = (sequencesData) => {
    // return a promise that indicates when we're all done or had an error
    return Promise.all(
      sequencesData.map((sequence) => {
        const formData = new FormData();
        formData.append("UserId", user.id);
        formData.append("ProcessTypeId", parseInt(sequence.ProcessTypeId));
        formData.append("DeliverableId", parseInt(sequence.DeliverableId));
        formData.append("DeliverableFile", sequence.DeliverableFile || "");
        formData.append("DeliverableLink", sequence.DeliverableLink || "");
        return axios.post(
          `https://hxstudiofileuploadv1.azurewebsites.net/api/FileUploadAPI/uploadprocessdiagram?userId=${user.id}`,
          formData,
          {
            headers: {
  ...
  const handleUpload = async (values) => {
    ...
    try {
      ...
      if (values.mockuptype === "Before After") {
        ...
        formData.append(
          `BeforeTags`,
          values.BeforeTags ? values.BeforeTags.join(",") : ""
        );
        formData.append(
          `AfterTags`,
          values.AfterTags ? values.AfterTags.join(",") : ""
        );
      }
      if (values.mockuptype === "Process Diagram & Artifacts") {
        ...
        values.deliverables.forEach((deliverable) => {
          if (
            ...
            (deliverable.files &&
              deliverable.files.file &&
              deliverable.files?.file?.name)
          ) {
            allProcessData.push({
              ProcessTypeId: values.process,
              DeliverableId: deliverable.deliverableId,
              DeliverableFile: deliverable.files?.file || '',
              DeliverableLink: deliverable.deliverableLink || "",
            });
          }
        });
        await uploadProcessData(allProcessData);
        fetchDeliverablesAndDiagrams()
      ...
      await axios.post(
        `https://hxstudiofileuploadv1.azurewebsites.net/api/FileUploadAPI/${
          ApiType[values.mockuptype]
        }?userId=${user.id}`,
        formData,
        {
          headers: {
  ...
  const handleFileUpload = (e, values, index, setFieldValue) => {
    const deliverables = values["deliverables"];
    const newFiles = Array.from(e.target.files);
    const newMockups = newFiles.map((file) => ({
      file,
    }));
    deliverables[index].files = newMockups[0];
    deliverables[index].isLink = false;
    deliverables[index].deliverableFileName = newMockups[0].file.name;
    deliverables[index].deliverableFilePath = '';
    deliverables[index].deliverableLink = '';
    setFieldValue("deliverables", deliverables);
  };
  const handleLink = (values, index, setFieldValue) => {
    const deliverables = values["deliverables"];
    delete deliverables[index].files;
    deliverables[index].deliverableFileName = '';
    deliverables[index].deliverableFilePath = '';
    deliverables[index].deliverableLink = "";
    deliverables[index].isLink = true;
    setFieldValue("deliverables", deliverables);
  };
  const handleDeleteFile = (values, index, setFieldValue) => {
    const deliverables = values["deliverables"];
    delete deliverables[index].files;
    deliverables[index].deliverableFileName = '';
    deliverables[index].deliverableFilePath = '';
  ...
  const getAllDeliverables = (id, setFieldValue) => {
    const ddata = deliverablesAllData.find((x) => x.processId === id)
    setFieldValue('deliverables', ddata.deliverables.map((x) => {
      const d = {
        ...x,
      }
      if (d.deliverableLink) {
        d.isLink = true;
      }
      return d
    }))
          ...
          {({ values, errors, touched, handleSubmit, setFieldValue }) => (
            <form onSubmit={handleSubmit}>
              <Modal.Header closeButton>
                <Modal.Title>
                  {values.id ? "Update Mock-up" : "Add New Mock-up"}
                </Modal.Title>
              </Modal.Header>
                    ...
                    <div className="d-flex gap-4">
                      {process.map((sec, index) => (
                        <Form.Check
                          ...
                          type="radio"
                          id={sec.id}
                          label={sec.processName}
                          name="process"
                          value={sec.id}
                          checked={values.process === sec.id}
                          onChange={() => {
                            setFieldValue("process", sec.id);
                            // const ddata = deliverablesData.filter(
                            //   (x) => x.processType.id === sec.id
                            // );
                            getAllDeliverables(sec.id, setFieldValue);
                            // setFieldValue("deliverables", ddata);
                          }}
                          className="me-3"
                        />
                      ))}
                  ...
                  <>
                    <Form.Group className="mb-3">
                      ...
                      <div
                        ...
                        onDrop={(e) =>
                          handleFileChange(e, values, "mockups", setFieldValue)
                        }
                      ...
                      {values.mockups.map((mockup, index) => (
                        <ListGroup.Item key={index} className="mb-3">
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            <strong>
                              {mockup.file ? mockup.file.name : mockup.name}
                            </strong>
                            <Button
                              ...
                              {mockup.tags?.map((tag, tagIndex) => (
                                ...
                                >
                                  ...
                                  <div
                                    ...
                                    style={{
                                      background:
                                        "#C2C2C2 0% 0% no-repeat padding-box",
                                      borderRadius: "50%",
                                      display: "flex",
                                      height: "20px",
                                      width: "20px",
                    ...
                    <Form.Group className="mb-3">
                      ...
                      {values.caseStudyFiles.length === 0 && (
                        <div
                          onClick={() =>
                            document
                              .getElementById("caseStudyFiles-file-input")
                              .click()
                          }
                          onDrop={(e) =>
                            handleFileChange(
                    ...
                    <Form.Group className="mb-3">
                      ...
                      {values.thumbnailImages.length === 0 && (
                        <div
                          onClick={() =>
                            document
                              .getElementById("thumbnailImages-file-input")
                              .click()
                          }
                          onDrop={(e) =>
                            handleFileChange(
                      ...
                      >
                        {values.tags?.map((tag, tagIndex) => (
                          ...
                          >
                            <span
                              className="tags"
                              style={{ marginRight: 3, marginTop: "-2px" }}
                            >
                              {tag}
                            </span>
                            <div
                    ...
                    <Form.Group className="mb-3">
                      ...
                      {values.BeforeFile.length === 0 && (
                        <div
                          onClick={() =>
                            document
                              .getElementById("BeforeFile-file-input")
                              .click()
                          }
                          onDrop={(e) =>
                            handleFileChange(
                    ...
                    <Form.Group className="mb-3">
                      ...
                      {values.AfterFile.length === 0 && (
                        <div
                          onClick={() =>
                            document
                              .getElementById("AfterFile-file-input")
                              .click()
                          }
                          onDrop={(e) =>
                            handleFileChange(
                          ...
                          >
                            <div className="gap-3">
                              ...
                              {item.isLink && (
                                <input
                                  type="text"
                                  className="project-title-input form-control"
                                  value={item.deliverableLink}
                                  onChange={(e) =>
                                    setFieldValue(
                                      `deliverables.${index}.deliverableLink`,
                                      e.target.value
                            ...
                            <div className="d-flex gap-3">
                              {/* <button
                                ...
                                onClick={() =>
                                  document
                                    .getElementById("file-input-data" + index)
                                    .click()
                                }
                                title="Upload"
                              >
                              ...
                              <BsLink45Deg
                                onClick={() =>
                                  handleLink(values, index, setFieldValue)
                                }
                                title="Link"
                              />
                            </div>
